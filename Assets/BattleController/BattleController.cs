...
public class BattleController : MonoBehaviour {
	...
	public string [] TeamsList1 = new string[2];
	...
	// public List <UnitIcon> UnitsIconList = new List<UnitIcon>();
	...
	public AttributeBoard attributes;
	...
	// 根据速度计算出待移动单位
	void GetUnitToMove() {
		UnitsList.Sort((x, y) => x.speed.CompareTo(y.speed));
		// UnitsList.Sort((x, y) => x.speed - y.speed);
		// Unit maxPos = null; // 位置最大的unit
		foreach (Unit unit in UnitsList) {
			if (hasSelecetedUnit == true) { // 当前有正在执行的棋
				PlayerTurn(); // 开始移动
				break;
			}
			// print(unit.unitIcon.transform.localPosition);
			unit.unitIcon.transform.Translate(Vector3.right * unit.speed * 10 * Time.deltaTime);
			if (unit.unitIcon.transform.position.x >= timeLineLength) {
				hasSelecetedUnit = true;
				selectedUnit = unit;
				// announcements.NextTurn (selectedUnit.name + "'s turn");
				var v = unit.unitIcon.transform.position;
				v.x =  unit.unitIcon.transform.position.x % timeLineLength;
				unit.unitIcon.transform.position = v;
				SetAttribute();
				break;
			}
		}
	...
	public void SetAttribute() {
		attributes.SetText("attack", selectedUnit.StatusList[0]);
		attributes.SetText("defend", selectedUnit.StatusList[1]);
		attributes.SetText("precise", selectedUnit.StatusList[2]);
		attributes.SetText("dodge", selectedUnit.StatusList[3]);
		attributes.SetText("strike", selectedUnit.StatusList[4]);
		attributes.SetText("life", selectedUnit.CurrentHP);
	...
	public void EndTurn(){
		...
		announcements.NextTurn ("Next Turn");
